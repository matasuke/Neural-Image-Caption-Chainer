...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--rnn_model_path', '-rm', type=str,
        default=os.path.join('..', 'data', 'models', 'rnn', 'STAIR_jp_256_Adam.model'),
        help="RNN model path"
    )
    parser.add_argument(
        '--cnn_model_path', '-cm', type=str,
        default=os.path.join('..', 'data', 'models', 'cnn', 'ResNet50.model'),
        help="CNN model path"
    )
    parser.add_argument(
        '--dict_path', '-d', type=str,
        default=os.path.join('..', 'data', 'vocab_dict', 'dict_STAIR_jp_train.pkl'),
        help="Dictionary path"
    )
    parser.add_argument(
        '--cnn_model_type', '-ct', type=str,
        choices=['ResNet', 'VGG16', 'AlexNet'], default="ResNet",
        help="CNN model type"
    )
    parser.add_argument(
        '--beamsize', '-b', type=int, default=3,
        help="beamsize"
    )
    parser.add_argument(
        '--depth_limit', '-dl', type=int, default=50,
    ...
    parser.add_argument(
        '--first_word', '-fw', type=str, default='<S>',
        help="set first word"
    )
    parser.add_argument(
        '--hidden_dim', '-hd', type=int, default=512,
        help="dimension of hidden layers"
    )
    parser.add_argument(
        '--mean', '-m', type=str, choices=['imagenet'], default='imagenet',
        help="method to preproces images"
    )
    parser.add_argument(
        '--img', '-i', type=str,
        default=os.path.join('..', 'sample_imgs', 'sample_img2.jpg'),
