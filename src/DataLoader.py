...
class DataLoader:
    def __init__(
            self,
            dataset,
            img_feature_root=os.path.join('..', 'data', 'images', 'features', 'ResNet50',),
            img_root=os.path.join('..', 'data', 'images', 'original'),
            img_mean="imagenet",
        ...
        if self.preload_features:
            self.img_features_train = np.array(
                [
                    np.load(
                        '{0}.npz'.format(
                            os.path.join(
                                self.img_feature_root,
                                os.path.splitext(image['file_path'])[0]
                            )
                        )
                    )['arr_0']
            ...
            if exist_test:
                self.img_features_val = np.array(
                    [
                        np.load(
                            '{0}.npz'.format(
                                os.path.join(
                ...
                self.img_features_test = np.array(
                    [
                        np.load(
                            '{0}.npz'.format(
                                os.path.join(
    ...
    def get_batch_train(self, batch_size=256, raw_img=False, raw_captions=False):
        ...
        if raw_img:
            batch_images = np.array(
                [
                    self.img_proc.load_img(
                        os.path.join(self.img_root, self.train_images[self.train_cap2img[i]]['file_path']),
                        expand_dim=False)
                    for i in batch_caption_indices
                ]
            )
        else:
            if self.preload_features:
                        ...
                        np.load(
                            '{0}.npz'.format(
                                os.path.join(
                                    self.img_feature_root,
                                    os.path.splitext(
                                        self.train_images[self.train_cap2img[i]]['file_path'])[0]
                                )
                            )
                        )['arr_0']
                        for i in batch_caption_indices]
        ...
        if raw_captions:
            ...
            batch_word_indices = [
                np.array(self.train_captions[i]['caption'], dtype=np.int32)
                for i in batch_caption_indices
            ]
    ...
    def get_batch_val(
        ...
        if raw_img:
            batch_images = np.array(
                [
                    self.img_proc.load_img(
                        os.path.join(
                            self.img_root,
                            self.val_images[self.val_cap2img[i]]['file_path']
                        ),
                        expand_dim=False
                    )
                    for i in batch_caption_indices
            ...
            if self.preload_features:
                batch_images = self.img_features_val[
                    [
                        self.val_cap2img[i]
                        for i in batch_caption_indices
                    ]
                ...
                batch_images = np.array(
                    [
                        np.load(
                            '{0}.npz'.format(
                                os.path.join(
                                    self.img_feature_root, os.path.splitext(
                                        self.val_images[self.val_cap2img[i]]['file_path']
                                    )[0]
                                )
                            )
        ...
        if raw_captions:
            ...
            batch_word_indices = [
                np.array(self.val_captions[i]['caption'], dtype=np.int32)
                for i in batch_caption_indices
            ]
    ...
    def get_batch_test(
        ...
        if raw_img:
            batch_images = np.array(
                [
                    self.img_proc.load_img(
                        os.path.join(
                            self.img_root,
                            self.test_images[self.test_cap2img[i]]['file_path']
                        ),
                        expand_dim=False
                    )
                    for i in batch_caption_indices
            ...
            if self.preload_features:
                batch_images = self.img_features_test[
                    [
                        self.test_cap2img[i]
                        for i in batch_caption_indices
                    ]
                        ...
                        np.load(
                            '{0}.npz'.format(
                                os.path.join(
                                    self.img_feature_root,
                                    os.path.splitext(
                                        self.test_images[self.test_cap2img[i]]['file_path'])[0]
                                )
                            )
                        )['arr_0']
                        for i in batch_caption_indices
